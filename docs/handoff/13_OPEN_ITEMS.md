...
1. **The ISO partner agreement (PDF)** — the operator's existing agreement, loaded into DocuSeal as the e-sign template (Phase 1). Builder maps the signature/field positions; no drafting.
2. **DNS access for the sending domains** — to set SPF/DKIM/DMARC and verify SES (Phase 0).
...
- Defaults accepted: Supabase Auth = partner identity; Tier A = VA phone/LinkedIn; Hetzner VPS; self-hosted DocuSeal.
